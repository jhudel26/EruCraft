...
const ResumeApp = {
    ...
    renderPreview() {
        ...
        if (config.paperSize === 'Letter') {
            ...
            preview.style.width = '215.9mm';
            preview.style.minHeight = '355.6mm';
        } else if (config.paperSize === 'Oficio') {
            preview.style.width = '215.9mm';
            preview.style.minHeight = '330.2mm';
        } else {
            ...
            <section class="tm-section">
                ...
                ${items.map(item => {
                    ...
                    <div style="margin-bottom: 1rem">
                        <div style="display:flex;justify-content:space-between;font-weight:600; color:#1e293b; font-size:1.05rem">
                            <span>
                                ${item.company || item.school || item.name || 'Untitled'} 
                                ${item.level ? `<span style="font-weight:500; color:#64748b; font-size:0.9rem">(${item.level})</span>` : ''}
                            </span>
                        ...
                        ${item.position ? `<div style="font-weight:500; color:#64748b; font-size:0.95rem; margin-top:0.1rem">${item.position}</div>` : ''}
                        ${item.degree ? `<div style="font-weight:500; color:#64748b; font-size:0.95rem; margin-top:0.1rem">${item.degree}</div>` : ''}
                        ${item.link ? `<div style="font-size:0.8rem; color:var(--primary); margin-top:0.1rem;"><i data-lucide="link" style="width:12px; height:12px"></i> <a href="${item.link}" target="_blank" style="color:inherit; text-decoration:none">${item.link}</a></div>` : ''}
